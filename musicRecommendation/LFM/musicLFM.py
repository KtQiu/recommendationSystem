...
def getUserNegativeItem(frame, userID):
    ...
    :return: 负反馈物品
    '''
    userItemlist = list(set(frame[frame['userID'] == userID]['songID']))  # 用户评分过的物品
    print('success!')
    # userItemlist = list(set( item, if item.value > 2 for item in frame[frame['userID'] == userID]['songID'] ))
    otherItemList = [item for item in set(frame['songID'].values) if item not in userItemlist]  # 用户没有评分的物品
    itemCount = [len(frame[frame['songID'] == item]['userID']) for item in otherItemList]  # 物品热门程度
...
def getUserPositiveItem(frame, userID):
    ...
    :return: 正反馈物品
    '''
    series = frame[frame['userID'] == userID]['songID']
    positiveItemList = list(series.values)
    print('Finish init positive!')
...
def initModel(frame, classCount):
    ...
    itemID = list(set(frame['songID'].values))
...
def recommend(frame, userID, p, q, TopN=10):
    ...
    :return: 推荐物品
    '''
    userItemlist = list(set(frame[frame['userID'] == userID]['songID']))
    otherItemList = [item for item in set(frame['songID'].values) if item not in userItemlist]
...
if __name__ == '__main__':
    target = 'b80344d063b5ccb3212f76538f3d9e43d87dca9e'
    frame = getResource('train.csv')
